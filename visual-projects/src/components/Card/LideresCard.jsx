...
export const LideresCard = () => {
  const dispatch = useDispatch();
  const { closeNavbar, nombreProyecto, proyecto, idLider, objProyectos } =
  ...
  //console.log(val)
  console.log(lideres)
  lideres.length===0 && dispatch(consultarLideres());
  ...
  const handleEliminarUsuario = async (event, rowData) => {
    try {
      ...
      const res = await axios
        .post("http://localhost/Apis/eliminar_lider.php", formData)
        .then((resJson) => {
  ...
  const handleAgregarLider = async (event, rowData) => {
    try {
      ...
      const res = await axios.post(
        "http://localhost/Apis/agregarLider.php",
        formData
      );
      console.log(res.data);
      Swal.fire(
        "Listo",
        "Se ha asignado el lider " +
          rowData.nombres +
          " al proyecto " +
          nombreProyecto,
        "success"
          ...
          <div className="container">
            <ContextMenuTrigger id="same_unique_identifier2">
              <MaterialTable
                ...
                actions={[
                  (rowData) => (
                    objProyectos.map((idlider) =>
                      setVal(idlider.idLider_proyecto)
                    ),
                    val != rowData.idLider_proyecto
                      ? {
                          ...
                          onClick: (event, rowData) =>
                            handleEliminarUsuario(event, rowData),
                        }
                      : {
                          icon: () => <AiFillDelete />,
                          tooltip:
                            "Primero desagrege al Lider del proyecto asosicado",
                          onClick: (event, rowData) =>
                            handleEliminarUsuario(event, rowData),
                          disabled: true,
                        }
...

                  {
                    ...
                    onClick: (event, rowData) =>
                      handleEditarUsuario(event, rowData),
                  },
                    ...
                    ? (rowData) =>
                        idLider == rowData.idLider_proyecto
                          ? {
                              ...
                              onClick: (event, rowData) =>
                                handleAgregarLider(event, rowData),
                            }
                          : {
